...
class TestSelectExpressionWithStrings(unittest.TestCase):
    ...
    def test_with_a_missing_selector(self):
        ...
        val, errs = self.bundle.format_pattern(self.bundle.get_message("foo").value, {})
        self.assertEqual(val, "B")
        self.assertEqual(errs, [FluentReferenceError("Unknown external: none")])
...
class TestSelectExpressionWithPluralCategories(unittest.TestCase):
    ...
    def test_with_cardinal_integer(self):
        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("count").value, {"num": 1}
        )
        self.assertEqual(val, "A")
        self.assertEqual(len(errs), 0)

        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("count").value, {"num": 2}
        )
        self.assertEqual(val, "B")
        self.assertEqual(len(errs), 0)
    ...
    def test_with_cardinal_float(self):
        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("count").value, {"num": 1.0}
        )
        self.assertEqual(val, "A")
        self.assertEqual(len(errs), 0)
    ...
    def test_with_ordinal_integer(self):
        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("order").value, {"num": 1}
        )
        self.assertEqual(val, "1st")
        self.assertEqual(len(errs), 0)

        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("order").value, {"num": 2}
        )
        self.assertEqual(val, "2nd")
        self.assertEqual(len(errs), 0)

        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("order").value, {"num": 11}
        )
        self.assertEqual(val, "11th")
        self.assertEqual(len(errs), 0)

        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("order").value, {"num": 21}
        )
    ...
    def test_with_ordinal_float(self):
        val, errs = self.bundle.format_pattern(
            self.bundle.get_message("order").value, {"num": 1.0}
        )
        self.assertEqual(val, "1st")
        self.assertEqual(len(errs), 0)
...
class TestSelectExpressionWithTerms(unittest.TestCase):
    ...
    def test_ref_term_attribute_missing(self):
        ...
        self.assertEqual(
            errs, [FluentReferenceError("Unknown attribute: -my-term.missing")]
        )
